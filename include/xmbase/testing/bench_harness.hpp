/*
 * testing/bench_harness.hpp — the family measurement harness.
 *
 * TEST/BENCH TIER ONLY: header-only, linked by test and benchmark binaries,
 * never by runtime (shipped) code.
 *
 * Unified at ADR 0007 W1 from the three prior generations — xmMessaging's
 * bench/harness.hpp (canonical: percentile math, hardware context, JSON
 * emitter, jitter fields), quickviz's ingestion bench_harness.hpp (display /
 * GL-renderer context, VmRSS probe, paced-rate and group/mode row fields),
 * and xmTelemetry's perf tier (the batched measurement loop discipline:
 * warmup, batch means, per-row batch bookkeeping).
 *
 * No google-benchmark: the family vendors none, and a benchmark dependency
 * would violate the no-new-external-deps rule for a job this small. What
 * the observability budget actually requires is here: tails (p50/p99/p99.9/
 * max, never means alone), hardware context embedded in every report, and a
 * machine-readable JSON artifact.
 *
 * Percentiles use the nearest-rank method on the sorted sample vector.
 * Micro benchmarks batch back-to-back ops per timed sample (amortizing the
 * ~20 ns clock read below the cost of the measured op — the telemetry
 * perf_budgets.cpp / ros2_tracing methodology); the batch size is recorded
 * in the report because a batched "max" is a max of batch MEANS. Paced /
 * per-event benchmarks time individual ops, so their tails are true
 * per-event tails.
 *
 * JSON compatibility: the emitted schema is a superset of the
 * xmmessaging-bench-v1 layout (bench/compare.py reads benchmarks[].name +
 * p50/p99/max — those names are load-bearing, do not rename them). Fields
 * added by the quickviz generation (group, mode, rate_hz, achieved_fps,
 * display, gl_renderer) are emitted only when set, so reports produced by
 * layer/payload-shaped suites stay byte-compatible with their consumers.
 *
 * Copyright (c) 2026 Ruixiang Du (rdu)
 */

#pragma once

#include <sys/utsname.h>
#include <unistd.h>

#include <algorithm>
#include <chrono>
#include <cmath>
#include <cstdint>
#include <cstdio>
#include <ctime>
#include <fstream>
#include <string>
#include <thread>
#include <utility>
#include <vector>

#include "xmbase/types/time.hpp"

namespace xmotion {
namespace testing {

// ---------------------------------------------------------------------------
// Statistics: tails, not means.
// ---------------------------------------------------------------------------

struct Stats {
  double p50 = 0.0;
  double p99 = 0.0;
  double p999 = 0.0;
  double max = 0.0;
  double mean = 0.0;
  std::size_t samples = 0;
};

// Nearest-rank percentile over a SORTED vector.
inline double PercentileSorted(const std::vector<double>& sorted, double q) {
  if (sorted.empty()) {
    return 0.0;
  }
  const auto rank = static_cast<std::size_t>(
      std::ceil(q * static_cast<double>(sorted.size())));
  const std::size_t index = rank == 0 ? 0 : rank - 1;
  return sorted[std::min(index, sorted.size() - 1)];
}

inline Stats ComputeStats(std::vector<double> samples) {
  Stats stats;
  stats.samples = samples.size();
  if (samples.empty()) {
    return stats;
  }
  std::sort(samples.begin(), samples.end());
  double sum = 0.0;
  for (double v : samples) {
    sum += v;
  }
  stats.p50 = PercentileSorted(samples, 0.50);
  stats.p99 = PercentileSorted(samples, 0.99);
  stats.p999 = PercentileSorted(samples, 0.999);
  stats.max = samples.back();
  stats.mean = sum / static_cast<double>(samples.size());
  return stats;
}

// ---------------------------------------------------------------------------
// Batched measurement loops (the telemetry perf-tier discipline, made
// reusable). AllocProbeT is any RAII counter exposing allocations() — pass
// xmotion::testing::AllocProbe from the ONE translation unit that includes
// alloc_probe.hpp, or leave the default no-op probe.
// ---------------------------------------------------------------------------

struct NullAllocProbe {
  std::uint64_t allocations() const noexcept { return 0; }
};

// Batched steady-clock sampling: each sample is the mean of `batch`
// back-to-back ops (amortizes the clock read). Warmup faults pages, warms
// caches, and settles the branch predictor before the probe starts —
// bench-code setup is exempt from the allocation gate; the measured section
// is not.
template <typename AllocProbeT = NullAllocProbe, typename Op>
Stats MeasureBatched(Op&& op, int batch, int samples,
                     std::uint64_t* allocations = nullptr) {
  for (int i = 0; i < batch * 4; ++i) {
    op();  // warmup
  }
  std::vector<double> per_op_ns(static_cast<std::size_t>(samples));
  {
    AllocProbeT probe;
    for (int s = 0; s < samples; ++s) {
      const ::xmotion::Timestamp t0 = ::xmotion::Now();
      for (int i = 0; i < batch; ++i) {
        op();
      }
      const ::xmotion::Timestamp t1 = ::xmotion::Now();
      per_op_ns[static_cast<std::size_t>(s)] =
          std::chrono::duration<double, std::nano>(t1 - t0).count() / batch;
    }
    if (allocations != nullptr) {
      *allocations = probe.allocations();
    }
  }
  return ComputeStats(std::move(per_op_ns));
}

// Variant with an UNPROBED, untimed per-sample setup (e.g. queue
// refill/drain between samples).
template <typename AllocProbeT = NullAllocProbe, typename Setup, typename Op>
Stats MeasureBatchedWithSetup(Setup&& setup, Op&& op, int batch, int samples,
                              std::uint64_t* allocations = nullptr) {
  setup();
  for (int i = 0; i < batch; ++i) {
    op();  // warmup
  }
  std::vector<double> per_op_ns(static_cast<std::size_t>(samples));
  std::uint64_t total_allocations = 0;
  for (int s = 0; s < samples; ++s) {
    setup();
    {
      AllocProbeT probe;
      const ::xmotion::Timestamp t0 = ::xmotion::Now();
      for (int i = 0; i < batch; ++i) {
        op();
      }
      const ::xmotion::Timestamp t1 = ::xmotion::Now();
      per_op_ns[static_cast<std::size_t>(s)] =
          std::chrono::duration<double, std::nano>(t1 - t0).count() / batch;
      total_allocations += probe.allocations();
    }
  }
  if (allocations != nullptr) {
    *allocations = total_allocations;
  }
  return ComputeStats(std::move(per_op_ns));
}

// ---------------------------------------------------------------------------
// One benchmark's report row. Core fields are the xmmessaging-bench-v1
// shape; optional fields from the quickviz generation are emitted only when
// set (see the header comment).
// ---------------------------------------------------------------------------

struct BenchResult {
  std::string name;   // e.g. "micro/latest_store/64B"
  std::string layer;  // e.g. "micro" | "path" | "system" | "contended"
  std::size_t payload_bytes = 0;
  bool contended = false;
  int batch = 1;  // ops per timed sample (1 = true per-event tails)
  Stats stats;    // nanoseconds
  std::uint64_t allocations = 0;  // measured-section allocations (probe)
  bool alloc_gated = false;  // true: allocations != 0 fails the run
  bool has_jitter = false;   // paced profiles only
  double jitter_p99_ns = 0.0;  // p99 of |inter-arrival - period|
  double jitter_max_ns = 0.0;
  // Optional row fields (quickviz generation) — emitted only when set.
  std::string group;   // e.g. "paced" | "render" | "observer" | "soak"
  std::string mode;    // e.g. "headless" | "windowed" | "stalled"
  double rate_hz = 0.0;        // producer pacing rate (0 = unpaced burst)
  double achieved_fps = -1.0;  // render-plane frame rate (< 0 = unset)
  std::string notes;
};

// ---------------------------------------------------------------------------
// Hardware context: a context-less report is a failing run.
// ---------------------------------------------------------------------------

struct HardwareContext {
  std::string cpu_model = "unknown";
  std::string governor = "unknown";
  std::string kernel = "unknown";
  bool preempt_rt = false;
  unsigned nproc = 0;
  double load_avg_1m = -1.0;
  double load_avg_5m = -1.0;
  double load_avg_15m = -1.0;
  // Optional context (quickviz generation) — emitted only when set.
  std::string display;      // e.g. ":0", "xvfb", "none"
  std::string gl_renderer;  // e.g. "llvmpipe"
};

inline std::string ReadFirstLine(const char* path) {
  std::ifstream in(path);
  std::string line;
  if (in && std::getline(in, line)) {
    return line;
  }
  return {};
}

inline HardwareContext CaptureHardwareContext() {
  HardwareContext hw;

  // CPU model: first "model name" row of /proc/cpuinfo.
  {
    std::ifstream cpuinfo("/proc/cpuinfo");
    std::string line;
    while (cpuinfo && std::getline(cpuinfo, line)) {
      if (line.rfind("model name", 0) == 0) {
        const auto colon = line.find(':');
        if (colon != std::string::npos) {
          auto value = line.substr(colon + 1);
          const auto first = value.find_first_not_of(" \t");
          hw.cpu_model =
              first == std::string::npos ? value : value.substr(first);
        }
        break;
      }
    }
  }

  const std::string governor =
      ReadFirstLine("/sys/devices/system/cpu/cpu0/cpufreq/scaling_governor");
  if (!governor.empty()) {
    hw.governor = governor;
  }

  struct utsname uts {};
  if (::uname(&uts) == 0) {
    hw.kernel = uts.release;
    hw.preempt_rt =
        std::string(uts.version).find("PREEMPT_RT") != std::string::npos;
  }
  // PREEMPT_RT also exposes /sys/kernel/realtime == "1".
  if (ReadFirstLine("/sys/kernel/realtime") == "1") {
    hw.preempt_rt = true;
  }

  hw.nproc = std::thread::hardware_concurrency();

  {
    std::ifstream loadavg("/proc/loadavg");
    if (loadavg) {
      loadavg >> hw.load_avg_1m >> hw.load_avg_5m >> hw.load_avg_15m;
    }
  }
  return hw;
}

// Current resident set size in KiB (VmRSS from /proc/self/status) — soak
// probes use it to catch slow growth. Returns -1 when unavailable.
inline long ReadVmRssKiB() {
  std::ifstream status("/proc/self/status");
  std::string line;
  while (status && std::getline(status, line)) {
    if (line.rfind("VmRSS:", 0) == 0) {
      long kib = 0;
      std::sscanf(line.c_str(), "VmRSS: %ld", &kib);
      return kib;
    }
  }
  return -1;
}

// ---------------------------------------------------------------------------
// JSON report writer. Hand-written emitter: the schema is small, fixed, and
// first-party — a JSON library would be a dependency for nothing.
// ---------------------------------------------------------------------------

inline void JsonEscapeTo(std::string& out, const std::string& value) {
  for (char c : value) {
    switch (c) {
      case '"':
        out += "\\\"";
        break;
      case '\\':
        out += "\\\\";
        break;
      case '\n':
        out += "\\n";
        break;
      case '\t':
        out += "\\t";
        break;
      default:
        if (static_cast<unsigned char>(c) >= 0x20) {
          out += c;
        }
        break;
    }
  }
}

inline std::string UtcTimestamp() {
  char buffer[32] = {};
  const std::time_t now = std::time(nullptr);
  std::tm tm_utc{};
  gmtime_r(&now, &tm_utc);
  std::strftime(buffer, sizeof(buffer), "%Y-%m-%dT%H:%M:%SZ", &tm_utc);
  return buffer;
}

// `schema` names the suite's report layout (e.g. "xmbase-bench-v1",
// "xmmessaging-bench-v1") so downstream tooling can dispatch on it.
inline bool WriteJsonReport(const std::string& path, const std::string& schema,
                            const HardwareContext& hw,
                            const std::vector<BenchResult>& results,
                            bool smoke, double scale,
                            const std::vector<std::string>& gate_failures) {
  std::string out;
  out.reserve(64 * 1024);
  char num[64];

  const auto field_str = [&](const char* key, const std::string& value,
                             bool trailing_comma = true) {
    out += '"';
    out += key;
    out += "\": \"";
    JsonEscapeTo(out, value);
    out += '"';
    if (trailing_comma) {
      out += ", ";
    }
  };
  const auto field_num = [&](const char* key, double value,
                             bool trailing_comma = true) {
    std::snprintf(num, sizeof(num), "%.1f", value);
    out += '"';
    out += key;
    out += "\": ";
    out += num;
    if (trailing_comma) {
      out += ", ";
    }
  };
  const auto field_int = [&](const char* key, long long value,
                             bool trailing_comma = true) {
    std::snprintf(num, sizeof(num), "%lld", value);
    out += '"';
    out += key;
    out += "\": ";
    out += num;
    if (trailing_comma) {
      out += ", ";
    }
  };
  const auto field_bool = [&](const char* key, bool value,
                              bool trailing_comma = true) {
    out += '"';
    out += key;
    out += "\": ";
    out += value ? "true" : "false";
    if (trailing_comma) {
      out += ", ";
    }
  };

  out += "{\n  ";
  field_str("schema", schema);
  field_str("generated_at_utc", UtcTimestamp());
  field_bool("smoke", smoke);
  field_num("scale", scale, false);
  out += ",\n  \"hardware\": { ";
  field_str("cpu_model", hw.cpu_model);
  field_int("nproc", hw.nproc);
  field_str("governor", hw.governor);
  field_str("kernel", hw.kernel);
  field_bool("preempt_rt", hw.preempt_rt);
  if (!hw.display.empty()) {
    field_str("display", hw.display);
  }
  if (!hw.gl_renderer.empty()) {
    field_str("gl_renderer", hw.gl_renderer);
  }
  field_num("load_avg_1m", hw.load_avg_1m);
  field_num("load_avg_5m", hw.load_avg_5m);
  field_num("load_avg_15m", hw.load_avg_15m, false);
  out += " },\n  \"alloc_gate\": { ";
  field_bool("passed", gate_failures.empty());
  out += "\"failures\": [";
  for (std::size_t i = 0; i < gate_failures.size(); ++i) {
    out += i == 0 ? "\"" : ", \"";
    JsonEscapeTo(out, gate_failures[i]);
    out += '"';
  }
  out += "] },\n  \"benchmarks\": [\n";
  for (std::size_t i = 0; i < results.size(); ++i) {
    const BenchResult& r = results[i];
    out += "    { ";
    field_str("name", r.name);
    field_str("layer", r.layer);
    if (!r.group.empty()) {
      field_str("group", r.group);
    }
    if (!r.mode.empty()) {
      field_str("mode", r.mode);
    }
    field_int("payload_bytes", static_cast<long long>(r.payload_bytes));
    field_bool("contended", r.contended);
    field_str("unit", "ns");
    if (r.rate_hz > 0.0) {
      field_num("rate_hz", r.rate_hz);
    }
    field_int("batch", r.batch);
    field_int("samples", static_cast<long long>(r.stats.samples));
    field_num("p50", r.stats.p50);
    field_num("p99", r.stats.p99);
    field_num("p999", r.stats.p999);
    field_num("max", r.stats.max);
    field_num("mean", r.stats.mean);
    field_int("allocations", static_cast<long long>(r.allocations));
    const bool more = r.has_jitter || r.achieved_fps >= 0.0 ||
                      !r.notes.empty();
    field_bool("alloc_gated", r.alloc_gated, more);
    if (r.has_jitter) {
      field_num("jitter_p99", r.jitter_p99_ns, true);
      field_num("jitter_max", r.jitter_max_ns,
                r.achieved_fps >= 0.0 || !r.notes.empty());
    }
    if (r.achieved_fps >= 0.0) {
      field_num("achieved_fps", r.achieved_fps, !r.notes.empty());
    }
    if (!r.notes.empty()) {
      field_str("notes", r.notes, false);
    }
    out += i + 1 < results.size() ? " },\n" : " }\n";
  }
  out += "  ]\n}\n";

  std::FILE* file = std::fopen(path.c_str(), "w");
  if (file == nullptr) {
    return false;
  }
  const bool ok = std::fwrite(out.data(), 1, out.size(), file) == out.size();
  return std::fclose(file) == 0 && ok;
}

}  // namespace testing
}  // namespace xmotion
