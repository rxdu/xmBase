 ...
 * placement.hpp
 *
 * Placement policy: where a concurrency primitive's cells live. The slot and
 * ring algorithms (latest_slot.hpp, bounded_queue.hpp) never say `new` — they
 * ask their Placement for storage, sized once at wiring time; nothing grows
 * afterwards.
 *
 * Promoted verbatim from xmMessaging detail/placement.hpp (ADR 0007, W1).
 * Requirement/decision IDs in comments (R7, P0b, P1b, M2/M4) are xmMessaging's,
 * retained so the rationale keeps its provenance.
 *
 * Policy contract — what LatestSlot / BoundedQueue require of a Placement P:
 *
 ...
 *     already hold live data that must be preserved (see RegionPlacement).
 *
 * The two policies:
 *
 *   - HeapPlacement: process-private heap cells, owning handles, always
 *     freshly zero-initialized (the P0b in-process reach).
 *   - RegionPlacement: cells are carved out of a pre-sized caller-provided
 ...
 * placement INSTANCE (stateless for heap, a region cursor for regions) so
...
// Region placement: a bump allocator over a fixed caller-provided region
...
struct RegionPlacement {
  ...
  RegionPlacement(unsigned char* base, std::size_t size,
 ...
 private:
  ...
  U* Carve(std::size_t count) {
    ...
    assert(cursor_ <= size_ &&
           "xmbase/concurrency: placement region overrun — layout constants "
           "disagree");
    if (initialize_) {
