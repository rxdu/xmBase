...
- **`xmbase/concurrency/`** (namespace `xmotion::concurrency`) — the family's verified concurrency primitives, promoted from xmMessaging `detail/` with their full verification transferring:
  - `MessageBuffer<T, Placement, Waiter>` — wait-free seqlock depth-1 exchange (Boehm construction; TSan-clean), with bounded crash-safe reads (`LoadBounded`) and sole-writer crash repair; `MutexMessageBuffer<T>` fallback for non-trivially-copyable values.
...
- **`xmbase/container/`** (`RingBuffer`, `ThreadSafeQueue`) — superseded by `xmbase/concurrency/` (`SpscQueue` for SPSC record queues). The byte-stream `RingBuffer` shape (burst span I/O + peek) has one consumer, xmDriver's `async_port`, which repatriates the type at W3.
